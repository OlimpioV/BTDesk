...
function onColDrop(e,colId){
  e.preventDefault();clearInds();
  if(dragColId){onColHeaderDrop(e,colId);return;}
  if(!dragCardId)return;
  var card=cards.find(function(c){return c.id===dragCardId;});if(!card){dragCardId=null;return;}
  var col=cards.filter(function(c){return c.status===colId&&c.id!==dragCardId;}).sort(function(a,b){return (a.ordem||0)-(b.ordem||0);});
  ...
  dragCardId=null;_dOvColId=null;_dOvIdx=null;
  renderKanban();
...
async function addColuna(){var nome=prompt("Nome da nova coluna:");if(!nome||!nome.trim())return;nome=nome.trim();var id="col_"+uid();var cc=COL_COLORS[COLS.length%COL_COLORS.length];COLS.push({id,label:nome,dot:cc.dot,cover:cc.cover,badgeBg:"#f1f5f9",badgeText:"#475569",ordem:COLS.length});try{await dbSaveCols();toast("Coluna criada!");}catch(e){toast("Erro",true);}renderKanban();}
async function delColuna(colId,e){e.stopPropagation();var col=COLS.find(function(c){return c.id===colId;});if(!col)return;if(cards.filter(function(c){return c.status===colId;}).length>0){toast("A coluna precisa estar vazia",true);return;}if(!confirm('Excluir "'+col.label+'"?'))return;COLS=COLS.filter(function(c){return c.id!==colId;});try{await dbSaveCols();toast("Coluna excluída!");}catch(err){}renderKanban();}
...
async function confirmDelCmt(cmtId){if(!confirm("Excluir este comentário?"))return;try{await delCmt(modalCardId,cmtId);toast("Excluído!");editingCmtId=null;renderModal();}catch(e){toast("Erro",true);}}
...
async function confirmDelCard(id){if(!confirm("Excluir esta demanda?"))return;var card=cards.find(function(c){return c.id===id;});try{await dbDel(id);await dbLog("Excluiu demanda",card?card.titulo:id);cards=cards.filter(function(c){return c.id!==id;});closeModal();renderView();}catch(e){toast("Erro",true);}}
...
function delEtq(n){if(!confirm('Excluir "'+n+'"?'))return;delete TC[n];if(TC_DEF[n])delete TC_DEF[n];TIPOS=Object.keys(TC);saveEtq();toast("Excluída!");editingEtq=null;renderEtq();}
...
async function delUser(id,nome){if(!confirm('Excluir "'+nome+'"?'))return;try{await dbDelUser(id);await dbLog("Excluiu usuário",nome);await loadResp();toast("Excluído!");renderUsers();}catch(e){toast("Erro",true);}}
...
async function saveCard(){
  ...
  try{await dbUpsert(card);await dbLog(editingId?"Editou demanda":"Criou demanda",titulo);if(editingId){cards=cards.map(function(c){return c.id===editingId?card:c;});}else cards.push(card);toast("Salvo!");editingId=null;renderView();}catch(e){toast("Erro",true);}
}
