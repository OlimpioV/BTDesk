...
async function dbDel(id){var r=await fetch(SB+"/rest/v1/demandas?id=eq."+id,{method:"DELETE",headers:H});if(!r.ok)throw new Error();}
...
async function dbDelTarefa(id){
  var r=await fetch(SB+"/rest/v1/tarefas?id=eq."+id,{method:"DELETE",headers:H});if(!r.ok)throw new Error();
}
async function dbDelTarefasDoCard(cardId){
  var r=await fetch(SB+"/rest/v1/tarefas?card_id=eq."+cardId,{method:"DELETE",headers:H});if(!r.ok)throw new Error();
}
...
async function dbFetchReunioesPorTarefas(tarefaIds){
  if(!tarefaIds||!tarefaIds.length)return [];
  var ids=tarefaIds.map(function(id){return String(id).replace(/,/g,"");}).join(",");
  var r=await fetch(SB+"/rest/v1/reuniao_tarefas?tarefa_id=in.("+ids+")&select=tarefa_id,reunioes(id,titulo,data,hora,status)&order=tarefa_id",{headers:H});
  if(!r.ok)return [];
  return r.json();
}
