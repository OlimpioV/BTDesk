...
function _reunTipo(r){
  if(r&&r.modelo_snapshot&&r.modelo_snapshot.cor)return {label:r.modelo_snapshot.nome,cor:r.modelo_snapshot.cor,ic:r.modelo_snapshot.icone||'users'};
  if(r&&r.modelo_id){var m=modelosDB.find(function(x){return x.id===r.modelo_id;});if(m)return {label:m.nome,cor:m.cor,ic:m.icone||'users'};}
...
function _renderReunioesPagina(){
  ...
  app.innerHTML=headerHTML("reunioes")
    ...
    +(perfil==="mestre"?'<button onclick="openGerenciarModelos()" class="rbtn rbtn-sm rbtn-ghost" style="width:100%;justify-content:center;">'+ic("palette")+' Gerenciar modelos</button>':"")
    +'</div>'
    +'</div>'
    +'<div class="reun-main">'
...
function _abrirFormReuniao(r){
  ...
  var modeloField='';
  if(modelosDB.length){var defId=r.modelo_id||(modelosDB.find(function(m){return m.slug==='acompanhamento';})||modelosDB[0]||{}).id||'';modeloField='<div class="field"><label>Modelo</label><select id="rf-modelo">'+modelosDB.map(function(m){return '<option value="'+m.id+'"'+(m.id===defId?' selected':'')+'>'+m.nome+'</option>';}).join("")+'</select></div>';}
...
async function salvarReuniao(id){
  ...
  var modeloEl=document.getElementById("rf-modelo");var modeloId=modeloEl?modeloEl.value||null:null;
  var m=modeloId?modelosDB.find(function(x){return x.id===modeloId;}):null;
  var rAtual=id?reunioesDB.find(function(x){return x.id===id;}):null;
  var modeloMudou=!id||(rAtual&&rAtual.modelo_id!==modeloId);
  var obj={data,hora:hora+":00",status,observacoes:obs||null,equipe_id,criado_por:userDbId};
  if(titulo)obj.titulo=titulo;
  if(id)obj.id=id;
  if(m&&modeloMudou){obj.modelo_id=m.id;obj.modelo_snapshot={nome:m.nome,cor:m.cor,icone:m.icone};if(m.slug)obj.tipo=m.slug;}
...
async function _loadPautasSection(reuniaoId){
  ...
  try{
    ...
    catOrder.forEach(function(catId){
      var cat=cats[catId];
      html+='<div>';
      html+='<div class="reun-grp-lbl" style="padding:0 2px 8px;">'+cat.nome+'</div>';
      html+='<div class="tcard-list">';
      ...
      if(ce&&!ehPassado){
        html+='<div style="margin-top:8px;">'
          +'<button class="rbtn rbtn-sm" onclick="openAdicionarPauta(\''+reuniaoId+'\')">'
          +ic("plus")+' Nova tarefa'
          +'</button></div>';
      }
      html+='</div>';
...
function _buildTarefaCard(t,ce,ehPassado){
  ...
  html+='<div class="tcard-bar" style="background:'+bar+';"></div>';
  html+='<div class="tcard-main">';

  // cabecalho do cartao
  html+='<div class="tcard-hdr">';
  html+='<div style="display:flex;align-items:flex-start;gap:6px;flex:1;min-width:0;">';
  html+='<button class="tcard-chev'+(subExp?' aberto':'')+'" onclick="_toggleSubExpand(\''+t.id+'\','+!!ehPassado+')" title="Expandir">&#9658;</button>';
  html+='<span id="tp-txt-'+t.id+'" class="tcard-title" style="'+(t.status==='concluido'?'text-decoration:line-through;color:var(--text3);':'')+(canEdit?'cursor:pointer;':'')+'"'
    +(canEdit?' onclick="event.stopPropagation();_iniciarEdicaoTitulo(\''+t.id+'\',false,null,'+!!ehPassado+')" title="Clique para editar"':'')+'>'+t.texto+'</span>';
  html+='</div>';
  html+='<div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">';
  ...
  // linha meta: responsavel + datas
  html+='<div class="tcard-meta">';
  if(t.responsavel){
    var u=(usuariosFullDB||[]).find(function(x){return x.sigla===t.responsavel;})||{};
    var ini=t.responsavel.slice(0,2).toUpperCase();
    html+='<div class="av av-sm" style="background:'+_avCor(u.id||t.responsavel)+';font-size:9px;width:22px;height:22px;min-width:22px;flex-shrink:0;">'+ini+'</div>';
    html+='<span>'+t.responsavel+'</span>';
  }
  if(t.data_inicio){html+='<span style="margin-left:'+(t.responsavel?'4px':'0')+';">'+_fmtDateBrShort(t.data_inicio)+'</span>';}
  if(t.data_fim){
  ...
  if(subExp){
    if(t.descricao){
      html+='<div id="tp-desc-'+t.id+'" style="font-size:12px;color:var(--text2);line-height:1.55;white-space:pre-wrap;margin-top:8px;'+(canEdit?'cursor:text;':'')+'"'
        +(canEdit?' onclick="event.stopPropagation();_iniciarEdicaoDescricaoMain(\''+t.id+'\','+!!ehPassado+')"':'')+'>'+t.descricao+'</div>';
    } else if(canEdit){
      html+='<div id="tp-desc-'+t.id+'" onclick="event.stopPropagation();_iniciarEdicaoDescricaoMain(\''+t.id+'\','+!!ehPassado+')" class="tp-add-desc-ph" style="font-size:12px;color:var(--text3);font-style:italic;cursor:text;margin-top:8px;">Adicionar descricao...</div>';
    }
  }

  // barra de progresso (sempre visivel quando ha subtarefas no cache)
  if(subtarefas&&subtarefas.length>0){
    var conc=subtarefas.filter(function(s){return s.status==='concluido';}).length;
    html+='<div class="prog" style="margin-top:10px;">';
    subtarefas.forEach(function(s){
      html+='<div class="prog-seg" style="background:'+(corSeg[s.status]||'#e8edf2')+';"></div>';
    });
    html+='</div>';
    html+='<div class="prog-info">'+conc+' de '+subtarefas.length+' itens concluídos</div>';
  }
  ...
  html+='</div>';// fecha tcard-main
  html+='</div>';// fecha tcard
  return html;
}
...
function openFormModelo(idOpcional){
  ...
  var mc=document.getElementById("modal-container");
  mc.innerHTML='<div class="modal-overlay" onclick="closeModal(event)"><div class="modal-box" onclick="event.stopPropagation()" style="width:min(95vw,480px);">'
    +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">'
    +'<div style="font-size:16px;font-weight:700;color:var(--bt-navy);font-family:var(--font-titulo);">'+(m?"Editar modelo":"Novo modelo")+'</div>'
    +'<button onclick="closeModal()" style="background:var(--surface);border:1px solid var(--border);color:var(--text3);padding:5px;border-radius:7px;cursor:pointer;">'+ic("close")+'</button>'
    +'</div>'
    +'<div class="field"><label>Nome</label><input id="mf-nome" value="'+(m?m.nome:'')+'"/></div>'
...
async function salvarModelo(){
  ...
  var obj={nome:nome,cor:_mfCor,icone:_mfIcone};
...
async function duplicarModelo(id){
  ...
  try{
    await dbUpsertModelo({nome:m.nome+' (cópia)',cor:m.cor,icone:m.icone,criado_por:userDbId});
    modelosDB=await dbFetchModelos();
